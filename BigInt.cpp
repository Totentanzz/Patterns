...
#include <fstream>
	...
	*/
	...
	Array<long long> BigInt::getArray() {
		return array;
	}
	...
	void BigInt::equalize(void* second) {
		...
		if (state == 1) {
			while (elemNum2 < elemNum1) {
				array.makeEqualSize(array.getSize(), secondBI->array.getSize()); //
	...
	*/
	void BigInt::add(void* added) {
		...
		if (sign == 1 && secondBi->sign == -1) {
			...
			equalize(added);
			for (int i = array.getElmNum() - 1, j = i; i >= 0; i--, j--) {
				long long curElem = array.getByIndex(i);
	...
	*/
	...
	void BigInt::swap(void* second) {
		BigInt* secondBi = (BigInt*)second;
		Array<long long>::swapArray(array, secondBi->array);
		short tmpSign = sign;
		sign = secondBi->sign;
	...
	Memento* BigInt::createMemento() {
		return new Memento(this->array, this->sign);
	}

	void BigInt::reinstate(Memento* mem) {
		this->array = mem->ar;
		this->sign = mem->si;
	}
